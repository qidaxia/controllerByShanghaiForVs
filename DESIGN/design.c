...
extern void handlerFrameTest(DEVICE com)
{
	...
	case 0x0D:
		...
		case 0x04://停止（大梁小车全部停止）
			if ((uint8_t)MotorMove(ID_DALIANG, stop) + (uint8_t)MotorMove(ID_DALIANG, stop) < 2)
			{
				SendBack(com, IS_ERROR);
			}
			break;
...
extern void handlerFrame(DEVICE com)
{
	...
	if (dataStr[len - 1] == 'd'&&dataStr[len - 2] == 'n'&&dataStr[len - 3] == 'e')
	{
		...
		{
			BeepOne();
			switch (dataStr[headIndex + 5])
			{
				//1-设置日期和时间
			case 0x0A: //年(67),月(8),日(9),时(10),分(11),秒(12),星期(13)（周日为7）
					   //RTC_Set_Date(u8 year,u8 month,u8 date,u8 week)	
				PWR_BackupAccessCmd(ENABLE);
				RTC_Set_Date((dataStr[6] + (dataStr[7] << 8)) - 2000, dataStr[8], dataStr[9], dataStr[13]);
				//RTC_Set_Time(u8 hour,u8 min,u8 sec,u8 ampm)
				RTC_Set_Time(dataStr[10], dataStr[11], dataStr[12], RTC_H12_AM);
				PWR_BackupAccessCmd(DISABLE);
				SendBack(com, IS_OK);
				break;
			...
			case 0x0C://行数(4个字节，LSB),列数(4个字节，LSB)
				write_ram_buf[0] = 0;
				write_ram_buf[1] = 0;
				write_ram_buf[2] = dataStr[7];
				write_ram_buf[3] = dataStr[6];
				map.Plant_Row = (dataStr[7] << 8) + dataStr[6];
				write_to_backup_sram(write_ram_buf, 4, 11); //行数

				write_ram_buf[0] = 0;
				write_ram_buf[1] = 0;
				write_ram_buf[2] = dataStr[11];
				write_ram_buf[3] = dataStr[10];
				map.Plant_Column = (dataStr[11] << 8) + dataStr[10];
				write_to_backup_sram(write_ram_buf, 4, 15); //列数
				SendBack(com, IS_OK);
				if (map.Plant_Column == 1)
					map.Column_Interval = 0;
			...
			case 0x01:
				if (ReadStatus(ID_XIAOCHE))
				{
					...
					write_ram_buf[2] = XiaoChe_Now_Position >> 8;
					write_ram_buf[3] = XiaoChe_Now_Position;
					write_to_backup_sram(write_ram_buf, 4, BASEADDR_ROW + 4 * (dataStr[6] - 1)); //行坐标
					SendBackValue(com, XiaoChe_Now_Position);
				}
				else
				{
			...
			case 0x0D:
				...
				case 0x04://停止（大梁小车全部停止）
					if (MotorMove(ID_DALIANG, stop) == RET_ERR || MotorMove(ID_DALIANG, stop) == RET_ERR)
					{
						SendBack(com, IS_ERROR);
					}
					break;
			...
			case 0x0F:
				...
				{
					Scan_Column(dataStr[7] + (dataStr[8] << 8));
				}
				WorkStatus = 0;
				break;
				...
				//计划任务扫描控制参数
			case 0x0B:
				paulseStyle = dataStr[6];//脉冲输出方式
				scanStartTime.Hour = dataStr[7];				//起始时间：时
				scanStartTime.Minute = dataStr[8];				//起始时间：分
				scanStopTime.Hour = dataStr[9];					//结束时间：时
				scanStopTime.Minute = dataStr[10];				//结束时间：分
				Scan_Interval = dataStr[11] + (dataStr[12] << 8); //扫描时间间隔
				scanRepeatStyle = (ScanRepeatStyle)dataStr[15];	//扫描重复方式
				...
				write_ram_buf[3] = scanStopTime.Hour;           //结束时间：时
				write_ram_buf[4] = scanStopTime.Minute;         //结束时间：分
				write_ram_buf[5] = dataStr[11];					 //扫描时间间隔
				write_ram_buf[6] = dataStr[12];
				write_ram_buf[7] = dataStr[13];
				write_ram_buf[8] = dataStr[14];
				write_ram_buf[9] = dataStr[15];					//扫描重复方式
				write_to_backup_sram(write_ram_buf, 10, 67); //写入数据RAM
				SendBack(com, IS_OK);
				break;
				...
				//行扫描位设定
			case 0x03:
				for (i = 0; i < dataStr[7] + 1; i++)
				{
					write_ram_buf[i] = dataStr[i + 7];	//数据长度rcv_buf[6]
				}
				write_to_backup_sram(write_ram_buf, dataStr[7] + 1, BASEADDR_COLUMN_BIT + 40 * (dataStr[6] - 1)); //起始地址100,每行最多40字节
				delay_ms(20);			//延时等待上次操作完成
				SendBack(com, IS_OK);
				break;
			...
			case 0x05:
				...
				//扫描任务开关
			case 0x06:
				Scan_SW = dataStr[6]; 	//参数1：1允许扫描，0禁止扫描
				write_ram_buf[0] = dataStr[6];
				write_to_backup_sram(write_ram_buf, 1, 77); //自动扫描任务开关
				SendBack(com, IS_OK);
				break;
			}
